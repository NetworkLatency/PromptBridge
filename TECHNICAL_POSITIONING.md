...
PromptBridge 是一个本地优先的多语言模型 Gateway。它通过结构化英文 Prompt 编译、
代码与术语保真、用户主动网页捕获、可切换的 OpenAI-compatible Provider Profile 和
metadata-only trace，帮助非英语用户把网页上下文和任务可靠地交给 API 或网页端强模型。
...
> 和上下文细节。因此我实现了 PromptBridge：编译模型先把用户意图转换成严格 PromptIR，
> 代码块和术语通过唯一 placeholder 保护；程序校验 JSON schema、动态语义字段以及每个
> placeholder，再按实际需求渲染英文执行 Prompt；代码等任务输入进入独立 Input Material，
> 不会被硬塞进目标句。网页选中文本作为不可信数据单独附加，
> 不参与语义重构。Provider 层按协议而不是平台名称抽象，所以
...
```text
用户原始任务与编译后的 PromptIR
用户选中的网页文本
精确匹配的术语
模型识别的 source language 元数据
```
...
## 4. Semantic Compilation，而不是全文翻译或固定模板

### 当前方法

PromptBridge 将输入按语义角色处理：

- 用户自然语言任务：编译成经过 schema 校验的英文 PromptIR；
- fenced code block：替换为不可变 placeholder；
- glossary term：精确匹配后锁定，可恢复为用户指定的规范英文词；
- page context：不参与任务重构，最后作为 untrusted reference data 附加。

PromptIR 只强制 `source_language` 和 `objective`。`context`、`input_material`、
`constraints`、`expected_deliverable` 和 `output_preferences` 根据当前请求条件渲染。稳定
schema 用于解析、测试和 trace；它不代表每个用户都必须接受同一种输出格式。
...
placeholder 是否恰好出现一次，同时拒绝未知 placeholder。缺失、重复或伪造时 fail closed，
这把保真从“希望模型听话”变成了程序检查。

### 为什么不使用固定 Constraints 和 Deliverable

默认要求“输出表格”“给出三条建议”看起来更专业，实际上会修改用户意图，也让不同任务
都长成同一种模板。PromptBridge 固定的是字段类型和章节顺序，内容只能来自当前请求；字段
为空时完全不渲染。V0 暂不读取长期用户偏好，因为当前还没有可靠的偏好存储与确认流程。
...
```text
compiler model -> validated English execution prompt
downstream model -> raw answer, with no response-language instruction
```

V0 选择英文作为统一 execution language，是一个需要评测的产品策略，不是“英文在所有模型、
语言和任务上必然更好”的普遍结论。现有多语言研究足以说明能力差距和 English-centric
transform 值得实验，但最终仍要与 source-language direct inference 做同模型、同任务对照。
当前 schema 可以确定性验证字段形状和 placeholder 保真，但不能仅靠 JSON 结构证明语义文本
一定是英文；这一点需要 compiler model 合规性测试和后续真实语料评测。
...
### 为什么不在 Prompt 中指定回答语言

要求下游模型直接使用用户母语回答，会把“英文执行是否有效”和“目标语言生成质量”重新
混在一次调用中，也与当前 English-first 假设冲突。因此 `--to` 已删除，最终 Prompt 不包含
`Response Language`。英文 Prompt 通常会引导英文回答，但 PromptBridge 接受模型实际返回的
任何语言，不把这一行为写成保证。

### 为什么 Response Reconstructor 是独立阶段

母语展示仍然是产品闭环的一部分，但职责应属于输出侧：保护代码、术语、链接和引用，只
翻译自然语言说明，再恢复不可变内容。当前 v0.4.0 尚未实现这个阶段，所以 `pb run` 返回
原始回答。把它作为下一独立里程碑，可以分别测试 Prompt 编译质量和回答翻译保真，而不是
一次提交两个难以归因的模型变换。
...
- 输入 hash 与长度；
- source language、固定 English prompt language 和动态 PromptIR 字段计数；
- glossary 命中；
- 失败阶段和 artifact 引用。

它不默认复制 prompt 和 response 正文。

Trace 是 metadata，但 artifact 包含完整 Prompt 和模型回答，因此存储卫生首先是隐私边界，
其次才是磁盘问题。`pb storage status` 是严格只读操作；`pb storage clean` 默认 dry-run，
只按 `trace_id` 删除可识别的 trace/artifact group，未知文件和配置始终保留。

当前没有硬编码自动 TTL。个人工具中的 artifact 可能是用户主动保留的工作成果，静默删除
同样会损害体验。先提供可观察、可预览、可测试的显式清理；浏览器闭环产生真实数据后，
再决定是否需要可配置 retention policy，而不是预设一个看似生产化的固定期限。
...
因此当前不实现 `eval replay`。现在只保留一个与核心风险直接相关的 compiler case suite：
6 个中文、英文、西班牙语、代码术语和不可信网页场景。离线模式用 golden PromptIR 验证
程序链路，live 模式显式确认后才调用真实模型，而且不写 artifact/trace。

自动检查只判断结构、关键词、动态章节和保真边界；人工再按 intent fidelity、constraint
fidelity、non-invention 和 English quality 四项各 0-2 分。当前不用另一个 LLM 自动打分，
因为小样本阶段 judge 本身的偏好和错误可能比被测差异更大，也会引入额外成本。
...
1. Schema-validated semantic compilation with segment protection。
...
- “实现了智能长期记忆”；
- “已经完成英文回答到用户母语的重构”；
- “英文 Prompt 在所有任务上一定更好”；
- “本地模型一定比直接推理效果好”。

推荐说：

> 这是 production-oriented personal tool。我实现了密钥隔离、协议归一化、有限重试、
> schema-validated PromptIR、fail-closed 内容保护、metadata trace，以及无 host permission
> 的选中文本捕获；response reconstruction、本机 loopback 通信、真实多语言评测和本地
> 模型基准仍是明确记录的后续阶段。
...
- Built PromptBridge, a local-first multilingual gateway that compiles non-English and mixed-
  language requests into validated English PromptIR while preserving code, terminology, and
...
- Replaced free-form prompt rewriting with strict JSON parsing, conditional task-specific sections,
  exact-once placeholder validation, and metadata-only semantic tracing.
